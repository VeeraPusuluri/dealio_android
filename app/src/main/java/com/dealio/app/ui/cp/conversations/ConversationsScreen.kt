...
fun ConversationsScreen(nav: NavController, vm: CpGrowthViewModel = viewModel()) {
    ...
    SubScreenScaffold("Conversations", nav) { inner ->
        ...
        // Each referred deal is a conversation thread the CP can open.
        val deals = state.leads.distinctBy { it.id }
        ...
        LazyColumn(
            ...
            item {
                Text(
                    "${deals.size} ${if (deals.size == 1) "conversation" else "conversations"}",
            ...
            if (deals.isEmpty()) {
                ...
                items(deals.size) { i ->
                    val d = deals[i]
                    DealioCard(onClick = { nav.navigate(CpRoutes.dealDetail(d.id)) }) {
                        Row(verticalAlignment = Alignment.CenterVertically) {
                            Box(
                                Modifier.size(44.dp).background(tintBrush(Teal), RoundedCornerShape(14.dp)),
                            ...
                            Column(Modifier.weight(1f)) {
                                ...
                                Row(verticalAlignment = Alignment.CenterVertically) {
                                    ...
                                    Text(d.projectName, color = TextSecondary, fontSize = 12.sp, maxLines = 1)
                                }
                            }
                            StatusChip(d.status.ifBlank { "Lead" })
                        }
                    }
                }
            }
